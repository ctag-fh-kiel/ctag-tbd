...
uint8_t CodecWM8::WM8978_Write_Reg(uint8_t reg, uint16_t val) {
    unsigned char buf[2];
...
uint16_t CodecWM8::WM8978_Read_Reg(uint8_t reg) {
...
//adcen:adc使能(1)/关闭(0)
//dacen:dac使能(1)/关闭(0)
void CodecWM8::WM8978_ADDA_Cfg(uint8_t dacen, uint8_t adcen) {
    uint16_t regval;
...
//auxen:aux开启(1)/关闭(0)
void CodecWM8::WM8978_Input_Cfg(uint8_t micen, uint8_t lineinen, uint8_t auxen) {
    uint16_t regval;
    regval = WM8978_Read_Reg(2); //读取R2
    if (micen)
...
void CodecWM8::WM8978_Output_Cfg(uint8_t dacen, uint8_t bpsen) {
    uint16_t regval = 0;
    if (dacen)
        regval |= 1 << 0; //DAC输出使能
    if (bpsen) {
...
void CodecWM8::WM8978_MIC_Gain(uint8_t gain) {
    gain &= 0X3F;
...
void CodecWM8::WM8978_LINEIN_Gain(uint8_t gain) {
    uint16_t regval;
    gain &= 0X07;
...
//gain:0~7,0表示通道禁止,1~7,对应-12dB~6dB,3dB/Step
void CodecWM8::WM8978_AUX_Gain(uint8_t gain) {
    uint16_t regval;
    gain &= 0X07;
    regval = WM8978_Read_Reg(47);              //读取R47
    regval &= ~(7 << 0);                      //清除原来的设置
...
void CodecWM8::WM8978_I2S_Cfg(uint8_t fmt, uint8_t len) {
    fmt &= 0X03;
    len &= 0X03;                                  //限定范围
...
//voll:左声道音量(0~63)
//volr:右声道音量(0~63)
void CodecWM8::WM8978_HPvol_Set(uint8_t voll, uint8_t volr) {
    voll &= 0X3F;
    volr &= 0X3F; //限定范围
    if (voll == 0)
...
//设置喇叭音量
//voll:左声道音量(0~63)
void CodecWM8::WM8978_SPKvol_Set(uint8_t volx) {
    volx &= 0X3F; //限定范围
    if (volx == 0)
...
void CodecWM8::WM8978_3D_Set(uint8_t depth) {
    depth &= 0XF;                 //限定范围
...
//dir:0,在ADC起作用
//    1,在DAC起作用(默认)
void CodecWM8::WM8978_EQ_3D_Dir(uint8_t dir) {
    uint16_t regval;
...
void CodecWM8::WM8978_EQ1_Set(uint8_t cfreq, uint8_t gain) {
    uint16_t regval;
    cfreq &= 0X3; //限定范围
    if (gain > 24)
        gain = 24;
...
//gain:增益,0~24,对应-12~+12dB
void CodecWM8::WM8978_EQ2_Set(uint8_t cfreq, uint8_t gain) {
    uint16_t regval = 0;
    cfreq &= 0X3; //限定范围
    if (gain > 24)
        gain = 24;
...
//gain:增益,0~24,对应-12~+12dB
void CodecWM8::WM8978_EQ3_Set(uint8_t cfreq, uint8_t gain) {
    uint16_t regval = 0;
    cfreq &= 0X3; //限定范围
    if (gain > 24)
        gain = 24;
...
//gain:增益,0~24,对应-12~+12dB
void CodecWM8::WM8978_EQ4_Set(uint8_t cfreq, uint8_t gain) {
    uint16_t regval = 0;
    cfreq &= 0X3; //限定范围
    if (gain > 24)
        gain = 24;
...
void setupI2SWM8974_or_I2SWM8978() {
    /*
