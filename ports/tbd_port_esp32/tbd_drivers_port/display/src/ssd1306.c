...
void ssd1306_master_init(SSD1306_t* dev)
{
	ssd1306_hal_master_init(dev);
}

void ssd1306_init(SSD1306_t * dev, int width, int height)
{
	ssd1306_hal_init(dev, width, height);
	// Initialize internal buffer
	for (int i=0;i<dev->_pages;i++) {
		// FIXME: will this work for different resolutions
...
void ssd1306_show_buffer(SSD1306_t * dev)
{
	for (int page=0; page<dev->_pages;page++) {
		ssd1306_hal_display_image(dev, page, 0, dev->_page[page]._segs, dev->_width);
	}
}
...
{
	ssd1306_hal_display_image(dev, page, seg, images, width);
...
ssd1306_display_text_x3(SSD1306_t * dev, int page, char * text, int text_len, bool invert)
{
	...
	for (int nn = 0; nn < _text_len; nn++) {
		...
		for (int yy = 0; yy < 3; yy++)	{ // for each group of 8 pixels high (y-direction)
			...
			ssd1306_hal_display_image(dev, page+yy, seg, image, 24);
...
{
	ssd1306_hal_contrast(dev, contrast);
}
...
void ssd1306_scroll_text(SSD1306_t * dev, const char * text, int text_len, bool invert)
{
	...
	while(1) {
		...
		ssd1306_hal_display_image(dev, dstIndex, 0, dev->_page[dstIndex]._segs, sizeof(dev->_page[dstIndex]._segs));
...
{
	ssd1306_hal_hardware_scroll(dev, scroll);
}
...
void ssd1306_wrap_arround(SSD1306_t * dev, ssd1306_scroll_type_t scroll, int start, int end, int8_t delay)
{
	...
	if (delay >= 0) {
		for (int page=0;page<dev->_pages;page++) {
			ssd1306_hal_display_image(dev, page, 0, dev->_page[page]._segs, 128);
...
void ssd1306_fadeout(SSD1306_t * dev)
{
	...
	for(int page=0; page<dev->_pages; page++) {
		...
		for(int line=0; line<8; line++) {
			...
			for(int seg=0; seg<128; seg++) {
				ssd1306_hal_display_image(dev, page, seg, image, 1);
