...
from .sound_plugin_parser import SoundPluginFinder, SoundPlugins
...
def write_plugin_factory_header(headers: Headers, sound_plugins: SoundPlugins, factory_header_path: Path) -> None:    
    ...
    template = env.get_template('sound_plugin_factory.hpp')
    header = template.render(sound_processors=sound_plugins, headers=headers)
    with open(factory_header_path, 'w') as f:
        f.write(header)    


def write_meta_class(headers: Headers, sound_plugins, meta_header_path: Path) -> None:    
    template_path = Path(__file__).parent / 'templates'
    env = ji.Environment(loader=ji.FileSystemLoader(template_path), autoescape=ji.select_autoescape())    

    template = env.get_template('sound_plugin_meta.hpp')
    header = template.render(sound_processors=sound_plugins, headers=headers)
    header_name = f'{meta_header_path}.hpp'
    with open(header_name, 'w') as f:
        f.write(header)    

    template = env.get_template('sound_plugin_meta.cpp')
    ...
    with open(f'{meta_header_path}.cpp', 'w') as f:
        f.write(header)  
