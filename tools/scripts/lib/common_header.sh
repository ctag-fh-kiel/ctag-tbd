...
# @brief determine platform system
#
# The system can be 'esp32' or 'desktop'.
#
# @arg $1 [path]   project directory
# @arg $2 [str]    platform name
#
tbd_platform_get_system() {
  local project_dir=$1
  local platform_name=$2
  local config_file="$project_dir/config/platforms/platform.$platform_name.json"

  local system=$(jq -r .config.system "$config_file") || echo ""
  if [ -z "$system" ]; then
    return 1
  fi
...
# @brief determine platform architecture
#
# For desktop this will always be 'desktop' for esp32 platforms it will return the specific chip.
#
# @arg $1 [path]   project directory
# @arg $2 [str]    platform name
#
tbd_platform_get_arch() {
  local project_dir=$1
  local platform_name=$2
  local config_file="$project_dir/config/platforms/platform.$platform_name.json"

  local arch=$(jq -r .config.arch "$config_file") || echo ""
  if [ -z "$arch" ]; then
    return 1
  fi
