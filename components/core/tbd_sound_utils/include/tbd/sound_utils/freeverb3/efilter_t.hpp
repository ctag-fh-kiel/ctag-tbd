...
public:
    ...
    inline _fv3_float_t processarc() {
        ...
        if (count++ > count_max) {
            ...
            _fv3_float_t arc_r = tbd::sound_utils::fastsqrt(re * re + im * im);
            //UNDENORMAL(arc_r);
            re /= arc_r;
            im /= arc_r;
        }
    ...
    void setFreq(_fv3_float_t fc) {
        ...
        arc_re = tbd::sound_utils::fastcos(theta);
        arc_im = tbd::sound_utils::fastsin(theta);
    }
...
public:
    ...
    void
    setRAHDSR(int32_t samples, _fv3_float_t _attack, _fv3_float_t _hold, _fv3_float_t _decay, _fv3_float_t _sustain,
              _fv3_float_t _release);
...
private:
    ...
    int32_t count, countTotal, attack, hold, decay, sustain, release;
    bool loopMode;
};
...
public:
     ...
     */
    void setParams(_fv3_float_t param1, int32_t length) {
        pfn1_param = param1;
...
private:
        ...
        while (l > 1) {
            ...
            for (c = 0; c < k; c++) {
                v[c * l + l / 2] = (v[c * l] + v[((c + 1) * l) % N]) / 2.0
                                   + 2.0 * r * ((_fv3_float_t) std::rand() - (_fv3_float_t) RAND_MAX / 2.0) /
                                     (_fv3_float_t) RAND_MAX;
    ...
    int32_t pfn1_length, pfn1_count;
