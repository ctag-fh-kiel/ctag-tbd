...
#include <tbd/sound_utils/polypad/ChordSynth.hpp>
#include <cstring>
#include <cstdio>
...
void ChordSynth::NoteOff() {
    adsr.Gate(false);
}

void ChordSynth::SetCutoff(const uint32_t &cutoff) {
    params_.filter_freq = cutoff;
    svf.set_frequency(cutoff);
}

void ChordSynth::SetResonance(const uint32_t &resonance) {
    params_.filter_reso = resonance;
    svf.set_resonance(resonance);
}

void  ChordSynth::Process(float *buf, const uint32_t &ofs) {
...
bool ChordSynth::IsDead() {
    return adsr.IsIdle();
}

void ChordSynth::SetFilterType(const SvfMode &mode) {
    mode_ = mode;
}

void ChordSynth::SetDetune(const uint32_t &detune) {
    for (auto &osc:v_osc) {
        osc.SetDetune(detune);
    }
}

void  ChordSynth::calcInversion(int8_t *ht_steps, const int16_t &chord, const int16_t &inversion,
...
float ChordSynth::GetTTL() {
    return adsr.GetOutput();
}

void ChordSynth::Hold() {
    adsr.Hold();
}

void ChordSynth::Init(const ChordSynth::ChordParams &params) {
    params_ = params;
