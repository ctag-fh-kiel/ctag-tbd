...
#include <tbd/sound_utils/freeverb3/efilter.hpp>
...
void FV3_(iir_1st)::setLPF_BW(fv3_float_t fc, fv3_float_t fs) {
    ...
    //fv3_float_t tan_omega_2 = tbd::sound_utils::fasttan((omega_2);
    fv3_float_t tan_omega_2 = tbd::sound_utils::fasttan(omega_2);
...
void FV3_(iir_1st)::setLPF_A(fv3_float_t fc, fv3_float_t fs) {
    //a2 = tbd::sound_utils::expf_fast(-1 * M_PI * fc / (fs / 2.));
    a2 = tbd::sound_utils::expf_fast(-1.f * M_PI * fc / (fs / 2.f));
    b1 = 1.f;
    b2 = .12f;
...
void FV3_(iir_1st)::setHPF_A(fv3_float_t fc, fv3_float_t fs) {
    a2 = tbd::sound_utils::expf_fast(-1 * M_PI * fc / (fs / 2.f));
    b1 = 1.f;
    b2 = -1.f;
...
void FV3_(iir_1st)::setLSF_A(fv3_float_t f1, fv3_float_t f2, fv3_float_t fs) {
    a2 = -1.f * tbd::sound_utils::expf_fast(-1.f * M_PI * f1 / (fs / 2.f));
    b1 = -1.f;
    b2 = tbd::sound_utils::expf_fast(-1 * M_PI * f2 / (fs / 2.));
}

void FV3_(iir_1st)::setHSF_A(fv3_float_t f1, fv3_float_t f2, fv3_float_t fs) {
    a2 = tbd::sound_utils::expf_fast(-1.f * M_PI * f1 / (fs / 2.f));
    b1 = -1.f;
    b2 = tbd::sound_utils::expf_fast(-1.f * M_PI * f2 / (fs / 2.f));
    fv3_float_t norm = (1.f - a2) / (b1 + b2);
    b1 *= norm;
    b2 *= norm;
}
...
void FV3_(iir_1st)::setHPFwLFS_A(fv3_float_t fc, fv3_float_t fs) {
    b1 = -1.f;
    b2 = tbd::sound_utils::expf_fast(-1.f * M_PI * fc / (fs / 2.f));
    a2 = -.12f;
...
void FV3_(iir_1st)::setPoleLPF(fv3_float_t fc, fv3_float_t fs) {
    ...
    fv3_float_t cos_omega = tbd::sound_utils::fastcos(omega);
...
void FV3_(dccut)::setCutOnFreq(fv3_float_t fc, fv3_float_t fs) {
    ...
    float c = tbd::sound_utils::fastsin(M_PI * _fc);
    gain = (sqrt3 - 2.f * c) / (c + sqrt3 * tbd::sound_utils::fastcos(M_PI * _fc));
}

fv3_float_t FV3_(dccut)::getCutOnFreq() {
    const float sqrt3 = 1.732050807568877f;
    //return std::atan(std::sqrt(3.) * (1. - gain * gain) / (1. + 4. * gain + gain * gain)) / M_PI;
    return tbd::sound_utils::fastatan(sqrt3 * (1.f - gain * gain) / (1.f + 4.f * gain + gain * gain)) / M_PI;
}
