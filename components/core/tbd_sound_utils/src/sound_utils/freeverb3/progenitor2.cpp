...
#include <tbd/sound_utils/freeverb3/progenitor2.hpp>
#include <tbd/sound_utils/freeverb3/fv3_type_float.h>
#include <tbd/sound_utils/freeverb3/fv3_ns_start.h>

// Fs = 34.125 kHz
const int32_t FV3_(progenitor2)::iAllpassLCo[] = {617, 535, 434, 347, 218, 162, 144, 122, 109, 74,};
const int32_t FV3_(progenitor2)::iAllpassRCo[] = {603, 547, 416, 364, 236, 162, 140, 131, 111, 79,};
const int32_t FV3_(progenitor2)::iAllpassCLCo[] = {430, 341, 264, 174,};
const int32_t FV3_(progenitor2)::iAllpassCRCo[] = {447, 324, 247, 191,};
const int32_t FV3_(progenitor2)::idxOutCo2[] = {110, 790, 2, 189, 121, 10, 480, 359, 103, 30, 26, 10, 780, 109, 1200,
                                                1310, 780, 10, 310, 800,};
...
void FV3_(progenitor2)::mute() {
    ...
    noise1.mute();
    for (int32_t i = 0; i < FV3_PROGENITOR2_NUM_IALLPASS; i++) {
        iAllpassL[i].mute();
        iAllpassR[i].mute();
    }
    for (int32_t i = 0; i < FV3_PROGENITOR2_NUM_CALLPASS; i++) {
        iAllpassCL[i].mute();
        iAllpassCR[i].mute();
    }
}
...
void
FV3_(progenitor2)::processreplace(fv3_float_t *inputL, fv3_float_t *inputR, fv3_float_t *outputL, fv3_float_t *outputR,
                                  int32_t numsamples) {
    switch (reverbType) {
    ...
    while (count-- > 0) {
        ...
        outL = allpassmL_17_18._process_dc(
                delayL_16._process(allpassmL_15_16._process_dc(lpfLdamp_11_12(outL), lfo, mnoise)), lfo * (-1.),
                mnoise * (-1.));
        outR = allpassmR_21_22._process_dc(
                delayR_ts._process(allpassmR_19_20._process_dc(lpfRdamp_13_14(outR), lfo * (-1.), mnoise * (-1.))), lfo,
                mnoise);

        delayL_37._process(
                allpass3L_34_37._process(delayL_31._process(allpass2L_25_27._process(delayL_23._process(outL))), lfo));
        ...
        delayR_58._process(
                allpass3R_52_55._process(delayR_49._process(allpass2R_43_45._process(delayR_40._process(outR))),
                                         lfo * (-1.)));
        ...
        Dout = delayL_23._get_z(iOutC[8]) * 0.469
               ...
               +
               (delayL_37._get_z(iOutC[10]) + allpass2L_25_27._get_z1(iOutC2[4]) + allpass2L_25_27._get_z2(iOutC2[6]) -
                allpass2R_43_45._get_z2(iOutC2[8])
                + allpass3L_34_37._get_z1(iOutC2[10]) + allpass3L_34_37._get_z2(iOutC2[12]) +
                allpass3L_34_37._get_z3(iOutC2[14])
        ...
        Bout = delayR_40._get_z(iOutC[2]) * 0.469
               + (delayR_49._get_z(iOutC[1]) - delayL_31._get_z(iOutC[3]) + delayR_49._get_z(iOutC[5]) -
                  delayL_37._get_z(iOutC[6])
                  - delayL_23._get_z(iOutC2[1]) - delayL_31._get_z(iOutC2[3])) * 0.219
               + (delayR_58._get_z(iOutC[4]) + allpass2R_43_45._get_z1(iOutC2[5]) + allpass2R_43_45._get_z2(iOutC2[7]) -
                  allpass2L_25_27._get_z2(iOutC2[9])
                  + allpass3R_52_55._get_z1(iOutC2[11]) + allpass3R_52_55._get_z2(iOutC2[13]) +
                  allpass3R_52_55._get_z3(iOutC2[15])
...
void FV3_(progenitor2)::setFsFactors() {
    ...
    for (int32_t i = 0; i < FV3_PROGENITOR2_OUT_INDEX; i++) {
