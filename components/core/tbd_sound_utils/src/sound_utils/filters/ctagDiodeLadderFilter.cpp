...
#include <cmath>
#include <tbd/sound_utils/filters/ctagDiodeLadderFilter.hpp>
#include <tbd/sound_utils/ctagFastMath.hpp>

void tbd::sound_utils::ctagDiodeLadderFilter::Init() {
    int i;
...
float tbd::sound_utils::ctagDiodeLadderFilter::Process(float in) {
    G[3] = 0.5f * g / gp1;
...
void tbd::sound_utils::ctagDiodeLadderFilter::SetCutoff(float cutoff) {
    cutoff_ = cutoff;
    float T = 1.f / fs_;
    float Tdiv2 = T / 2.0f;
...
void tbd::sound_utils::ctagDiodeLadderFilter::SetResonance(float resonance) {
    kval_ = resonance * 16.998f;
}

void tbd::sound_utils::ctagDiodeLadderFilter::SetSampleRate(float fs) {
    fs_ = fs;
}
