...
void ctagBiQuad::SetType(BIQUAD_TYPE t) {
    type = t;
}

void ctagBiQuad::Mute() {
    w[0] = w[1] = 0.f;
}

/*
void ctagBiQuad::Process(float *data, const uint32_t sz, const uint32_t inc) {

}

float ctagBiQuad::Process(float data) {
    return 0;
}
*/
void ctagBiQuad::SetCutoffHz(float cut) {
    _cut = cut;
    calcCoeffs();
}

void ctagBiQuad::SetQ(float q) {
    _q = q;
    calcCoeffs();
}

void ctagBiQuad::SetSampleRate(float fs) {
    _fs = fs;
    calcCoeffs();
}

void ctagBiQuad::calcCoeffs() {
    switch (type) {
        case BIQUAD_TYPE::LP:
