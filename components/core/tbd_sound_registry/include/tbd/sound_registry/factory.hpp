...
template<class T>
Error activate_sound_processor(channels::ChannelMapping);

template<class T>
struct meta_for_sound_processor;

using Activator = Error (*)(channels::ChannelMapping);
extern Activator ACTIVATORS[];

inline Error set_plugin(const channels::ChannelMapping channels, const parameters::PluginID plugin_id) {
