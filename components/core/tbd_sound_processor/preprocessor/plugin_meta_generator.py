...
def is_plugin_class(cls: cpplib.ClassScope) -> bool:
    """ All plugins must be direct decendatns of `ctagSoundProcessor`. """
...
def is_in_list(plugin: ReflectableDescription, plugin_list: list[str]):
    ...
    if cls_name.lower().startswith('ctagsoundprocessor'):
        possible_names += get_name_variants(plugin.cls_name[18:])
