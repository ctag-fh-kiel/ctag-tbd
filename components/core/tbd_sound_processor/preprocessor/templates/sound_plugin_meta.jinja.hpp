...
class {{ meta_name }} : public {{ plugin.full_name }}, public sound_processor::DynamicSoundProcessor {
    ...
    virtual void Process(const CTAG::SP::ProcessData& data) {
        process_updates();
        populate_parameters(data);
        {{ plugin.name }}::Process(data);
    }

    void populate_parameters(const CTAG::SP::ProcessData& data) {
...
private:
    ...
    using IntSetter = void(*)({{ meta_name }}&, CTAG::SP::int_par);
    static IntSetter int_setters[{{ plugin.num_ints }}];

    using UIntSetter = void(*)({{ meta_name }}&, CTAG::SP::uint_par);
    static UIntSetter uint_setters[{{ plugin.num_uints }}];
    
    using TriggerSetter = void(*)({{ meta_name }}&, CTAG::SP::trigger_par);
