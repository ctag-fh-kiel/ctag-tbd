...
#include <tbd/sounds/SoundProcessorSubSynth.hpp>
#include <iostream>
#include <cmath>
...
void SoundProcessorSubSynth::Init(std::size_t blockSize, void *blockPtr) {
    isStereo = false;
...
void SoundProcessorSubSynth::Process(const audio::ProcessData&data) {
    ...
    for (uint32_t i = 0; i < bufSz; i++) {
        ...
        data.buf[i * 2 + processCh] = sound_utils::fasttanh(data.buf[i * 2 + processCh]);
    }
}
...
void SoundProcessorSubSynth::setParamValueInternal(const string &id, const string &key, const int val) {
...
void SoundProcessorSubSynth::loadPresetInternal() {
// autogenerated code here
// sectionCpp1
...
void SoundProcessorSubSynth::computeFilterCoefs(float *coeffs,
                                                    float freq,
                                                    float bw,
                                                    float gain) {
    ...
    float sn = sound_utils::fastsin(omega);
    float cs = sound_utils::fastcos(omega);
    float alpha = sn * sound_utils::fastsinh(LOG_2 / 2.0f * bw * omega / sn);
...
float SoundProcessorSubSynth::computeRolloff(float freq) {
    ...
    if (freq <= lower_limit || freq >= upper_limit)
        return 0.0f;
    if (freq <= lower_limit + lower_width)
        return (1.0f - sound_utils::fastcos(M_PI * (freq - lower_limit) / lower_width)) / 2.0f;
...
void SoundProcessorSubSynth::fetchControlData(const audio::ProcessData&data) {
    ...
    if (cv_root_frequency == -1)
        ...
        fRootFrequency = 0.9f * fRootFrequency +
                         0.1f * (float) root_frequency * sound_utils::fastpow2(data.cv[cv_root_frequency] * 5.f);
    if (cv_root_level == -1)
    ...
    for (int i = 0; i < 9; i++) {
        if (cv_p_harm[i] == -1)
            fHarm[i] = sound_utils::fastpow2((float) p_harm[i] / 12.f);
        else
            fHarm[i] = sound_utils::fastpow2(data.cv[cv_p_harm[i]] * 9.f);
