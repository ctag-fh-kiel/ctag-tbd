...
#include <tbd/sounds/SoundProcessorKarpuskl.hpp>
#include <cstdlib>
...
float lfo_val = 0.f;
  ...
  if( lfo_amount != 0.f )   // If amount is zero, LFO is shut off...
  {
    ...
    {
      modFreq *= sound_utils::fasterpow2(lfo_val * lfo_amount);     // We use a 2-power to have a somewhat logarithmic scaling of our control-knob/slider...
      if (modfreq_hi)    // As with the Moogerfooger MF-102 we have two different ranges of Carrier-Frequencies to choose from - this allows smoother transition over the whole range...
      {
  ...
  for (uint32_t i = 0; i < bufSz; i++)
  {
    ...
    if( driveCharacter > 0.f)
    {
      f_valB = f_valA;
      if (f_valB > 0.f)
        f_valB = 1.f - sound_utils::fasterexp(-1.f*f_valB)/1.5f;     // distort "to max amplitude", see https://dsp.stackexchange.com/questions/13142/digital-distortion-effect-algorithm
      else
      {
        if (f_valB < 0.f)
          f_valB = -1.f + sound_utils::fasterexp(f_valB)/1.5f;          // distort "to min amplitude", the natural logarithm ensures smoothing out on lower levels...
      }
...
void SoundProcessorKarpuskl::Init(std::size_t blockSize, void *blockPtr)
{
...
SoundProcessorKarpuskl::~SoundProcessorKarpuskl()
{
