...
#include <tbd/sounds/SoundProcessorStrampDly.hpp>
#include <iostream>
#include <cmath>
...
void SoundProcessorStrampDly::Init(std::size_t blockSize, void *blockPtr) {
    // config defs
    msMaxLength = 4095.f;
    fFeedback = 0.f;
    msLength = length;
...
void SoundProcessorStrampDly::Process(const sound_processor::ProcessData&data) {
    ...
    for (uint32_t i = 0; i < bufSz; i++) {
        ...
        data.buf[i * 2] = mFac * sound_utils::fasttanh(data.buf[i * 2] * fGain);
        data.buf[i * 2 + 1] = mFac * sound_utils::fasttanh(data.buf[i * 2 + 1] * fGain);
    }
}

SoundProcessorStrampDly::~SoundProcessorStrampDly() {
    heaps::free(bufL);
    heaps::free(bufR);
}

void SoundProcessorStrampDly::mute() {
