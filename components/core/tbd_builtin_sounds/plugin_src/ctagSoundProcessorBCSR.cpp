...
#include <tbd/sounds/SoundProcessorBCSR.hpp>
#include <iostream>
#include <cmath>
...
void SoundProcessorBCSR::Init(std::size_t blockSize, void *blockPtr) {
    // inits
    prevSample = 0.f;
    phase = 0.f;
...
void SoundProcessorBCSR::Process(const sound_processor::ProcessData&data) {
    ...
    for (int i = 0; i < bufSz; ++i) {
        ...
        if (isBitCrush) {
            ...
            crush = sound_utils::fastpow2(crush);
            if (sound_utils::fastfabs(out) < 1.f / crush) out = 0.f;
            out = floorf(out * crush) / crush;
        }

        out = sound_utils::fasttanh(out); // saturate
...
void SoundProcessorBCSR::updateParams(const sound_processor::ProcessData&data) {
    ...
    // Bit crush params
    if (cv_bc_amount != -1) {
        fScale = 16.f - sound_utils::fastfabs(data.cv[cv_bc_amount]) * 15.f;
    } else {
