...
#include <tbd/sounds/SoundProcessorTBDeep.hpp>
...
void SoundProcessorTBDeep::Init(std::size_t blockSize, void *blockPtr) {
...
void SoundProcessorTBDeep::Process(const sound_processor::ProcessData&data) {
    ...
    for (int j = 0; j < bufSz; j++) {
        ...
        data.buf[j * 2] = sound_utils::fasttanh(out[j].channel[o0] * loud0);
        data.buf[j * 2 + 1] = sound_utils::fasttanh(out[j].channel[o1] * loud1);
    }
}
