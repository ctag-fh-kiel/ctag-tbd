...
#include <tbd/sounds/SoundProcessorDust.hpp>
#include <iostream>
#include <cmath>
...
void SoundProcessorDust::Init(std::size_t blockSize, void *blockPtr) {
    dust.SetParams(0.5f, 1.0f, 0.0f, 44100.f);

    for (uint32_t i = 0; i < 6; i++) {
        coeffs[i] = 1.f;
    }
    tbd::sound_utils::dsps_biquad_gen_bpf0db_f32(coeffs, 0.002, 0.4f);
    w1[0] = 0.f;
    w1[1] = 0.f;
    w2[0] = 0.f;
    w2[1] = 0.f;
...
void SoundProcessorDust::Process(const sound_processor::ProcessData&data) {
    ...
    tbd::sound_utils::dsps_biquad_gen_bpf0db_f32(coeffs, fBPCut, fBPQ);
