...
inline int ctagSoundProcessorFreakwaves::process_param_trig(const ProcessData &data, int trig_myparm, int my_parm, int enum_trigger_id, int gate_type = 0 )
{
  int trig_status = 0;
...
void ctagSoundProcessorFreakwaves::prepareWavetable( const int16_t **wavetables, int currentBank, bool *isWaveTableGood, float **fbuffer, int16_t **ibuffer )
{
...
void ctagSoundProcessorFreakwaves::Process(const ProcessData &data)
{
  ...
  MK_TRIG_PAR(t_QuantInputA, QuantInputA);
  MK_INT_CONSTRAIN(iQscale_A, Qscale_A, 47);
  MK_TRIG_PAR(t_QuantInputB, QuantInputB);
  MK_INT_CONSTRAIN(iQscale_B, Qscale_B, 47);
  MK_INT_CONSTRAIN(iQscale_C, Qscale_C, 47);
  ...
  // --- LFOs ---
  MK_TRIG_PAR_LOC(t_lfoActive[0], lfoActive_1);
  MK_INT_CONSTRAIN_LOC(i_lfoDestination[0], lfoDestination_1,17);   // Destinations 0-17 are currently available
  ...
  MK_INT_CONSTRAIN_LOC(i_lfoDestination[3], lfoDestination_4,17);
  MK_INT_CONSTRAIN_LOC(i_lfoType[3], lfoType_4,6);
  ...
  if( t_Porta_A )
  {
    ...
    interpolate_hysteresis_vari(f_pitch_A, f_pitch_A, f_PortamentoTimeA, portamentoHyteresisA_);  // should work without the reset obove, too
  }
  if( t_Porta_B )
  {
    ...
    interpolate_hysteresis_vari(f_pitch_B, f_pitch_B, f_PortamentoTimeB, portamentoHyteresisB_);  // should work without the reset obove, too
  }
  // --- External modulation, see Modifier section ---
  if (t_ExtModCisOn)
  {
    // --- Calculate modulation caused by external input, if any ---
    interpolate_hysteresis(f_AmountForExternalMod, f_externalVal_ * f_AmountForExternalMod * f_ExtModGain,  externalHyteresis_);
  ...
  if (t_GeneratePitchC)
  {
    ...
    {
      if (!(fabsf(f_pitch_A - f_pitch_B) <  0.05f))  // Pitch of OSC A and OSC B are NOT identical within a range of less than 5 Cent
      {
    ...
    {
      if (fabsf(f_pitch_A - f_pitch_B) < 0.05f)  // Pitch of OSC A and OSC B are identical within a range of less than 5 Cent
      {
        f_pitch_stored_C_ = f_pitch_A;   // Set pitch of OSC C and also remember it internally as "most recent valid pitch"
        if (t_UseABGate)   // Only apply gate on OSC C if we encountered a gate for OSC A or OSC B
        {
  ...
  if( t_Porta_C )
  {
    ...
    interpolate_hysteresis_vari(f_pitch_stored_C_, f_pitch_stored_C_, f_PortamentoTimeC, portamentoHyteresisC_);  // should work without the reset obove, too
  }
  // --- Calculate LFO of modifier section ---
  lfoPitchC.SetFrequency(f_LFOspeedC);  // Please note: Pitch mod will also happen if we have no external signal to increase or decrease it!
  f_pitch_stored_C_ += lfoPitchC.Process() * f_LFOamountC * 7.f;  // We allow pitchmodulation (sine LFO) of max a fifht.
  ...
  if( t_AccentModCisOn )
  {
    ...
    {
      f_accent_target *= 7.f;
      CONSTRAIN(f_accent_target, -7.f, 7.f);                    // Max Accent Bend is a perfect fifth (7 semitones)
  ...
  if (t_QuantInputA ||  (!t_QuantInputA && lfoPitchedA))   // Pitch-quantize input and/or quantize modulation , index 0 does not quantize
  {
    quantiz_A.Configure( braids::scales[iQscale_A]); // 48 different scales, Braids uses integers, we have to multiply our floats by 128 below!
    f_pitch_A = quantiz_A.Process((int)(averageA.dejitter(f_pitch_A+f_tune_A) * 128.f), (int)(f_MasterPitch * 128.f)) /  128.f;  // possible negative values are "fixed" in the quantizer
  }
  if (t_QuantInputB || (!t_QuantInputB && lfoPitchedB))  // Pitch-quantize input and/or quantize modulation , index 0 does not quantize
  {
    quantiz_B.Configure( braids::scales[iQscale_B]); // 48 different scales, Braids uses integers, we have to multiply our floats by 128 below!
    f_pitch_B = quantiz_B.Process((int)(averageB.dejitter(f_pitch_B+f_tune_B) * 128.f), (int)(f_MasterPitch * 128.f)) /  128.f;  // possible negative values are "fixed" in the quantizer
  }
  // Pitch-quantize OSC C, index 0 does not quantize
  quantiz_C.Configure(braids::scales[iQscale_C]); // 48 different scales, Braids uses integers, we have to multiply our floats by 128 below!
  f_pitch_stored_C_ = quantiz_C.Process((int)(averageC.dejitter(f_pitch_stored_C_) * 128.f), (int)(f_MasterPitch * 128.f)) / 128.f;  // possible negative values are "fixed" in the quantizer
  ...
  float totalPitchA = t_QuantInputA ? f_pitch_A : f_pitch_A+f_tune_A; // If we quantize the incoming pitch, tuning is already part of the quantisation!
  const float f_freq_A = plaits::NoteToFrequency(60 + totalPitchA + f_current_note) * 0.998f;

  // --- Wave select B ---
  currentBank_B = WaveTblB;
  if (lastBank_B != currentBank_B)
  { // this is slow, hence not modulated by CV
    prepareWavetable((const int16_t **) &wavetables_B, currentBank_B, &isWaveTableGood_B, &fbuffer_B, &buffer_B);
    lastBank_B = currentBank_B;
  }
  float totalPitchB = t_QuantInputB ? f_pitch_B : f_pitch_B+f_tune_B; // If we quantize the incoming pitch, tuning is already part of the quantisation!
  ...
  const float f_freq_C = plaits::NoteToFrequency(60 + f_relative_tune_C + f_pitch_stored_C_ + f_current_note) * 0.998f;
  ...
  if( t_ResonatorOn )
  {
    resonator_dry = (1.f - f_ResonatorDryWet)*f_Volume; // We multiply by volume already here as an optimisation, this will be the last result of the DSP-chain...
    resonator_wet = f_ResonatorDryWet*f_Volume;         // We multiply by volume already here as an optimisation, this will be the last result of the DSP-chain...
  }
  ...
  if(t_AddDelayAfterResonator)
  {
    ...
    for (uint32_t i = 0; i < 32; i++)
    {
      ...
      wave_osc_buf[i] = (oscillators_signal+external_signal_wet)*resonator_dry;   // Apply Resonator now and let original signal through partly if activated
    }
    resonator.Process(f_ResonatorFreq, f_ResonatorStructure, f_ResonatorBrightness, f_ResonatorDamping, reso_buf, reso_buf, bufSz);
    for (uint32_t i = 0; i < 32; i++)
      wave_osc_buf[i] = wave_osc_buf[i]+reso_buf[i]*resonator_wet;   // Apply Resonator now and let original signal through partly if activated
    ...
    if( t_CoutOnRightCh ) // Output for processed OSC C on the right, unprocessed OSC A+OSC B+external in on the left
    {
      ...
      {
        data.buf[bufId * 2] = delay_buf[bufId] * f_delay_wet + wave_osc_buf[bufId] * f_delay_dry + data.buf[bufId * 2] * f_ExternalDry;
        data.buf[bufId*2+1] = wave_osc_buf_c[bufId];
      }
      // === Postprocessing ===
      f_accentVal_ = averageAccent.dejitter((fabsf(data.buf[30])+fabsf(data.buf[31]))/2.f);  // Remember average volume of last completely processed value to maybe apply as Accent Bend or "Wavescan" in next round...
    }
  ...
  else // Add Delay before Resonator
  {
    ...
    if( t_CoutOnRightCh ) // Output for processed OSC C on the right, unprocessed OSC A+OSC B+external in on the left
    {
      for (uint32_t bufId = 0; bufId < bufSz; bufId++)
      {
        delay_buf[bufId] = delay_buf[bufId] * f_delay_wet + wave_osc_buf[bufId] * f_delay_dry;     // Add Wavefolder+delay as set by Dry/Wet Slider
...
maxDelayLength(88200), dlyLine(maxDelayLength)
{
  ...
  env_A.SetSampleRate(44100.f/ bufSz);       // ADSR-EG: sync Env with our audio-processing, but update only once outside the main DSP-Loop
  env_A.SetModeExp();                             // Logarithmic scaling
  env_B.SetSampleRate(44100.f/ bufSz);       // ADSR-EG: sync Env with our audio-processing, but update only once outside the main DSP-Loop
  env_B.SetModeExp();                             // Logarithmic scaling
  env_C.SetSampleRate(44100.f/ bufSz);       // ADSR-EG: sync Env with our audio-processing, but update only once outside the main DSP-Loop
...
ctagSoundProcessorFreakwaves::~ctagSoundProcessorFreakwaves()
{
  ...
  // Free mem for one wavetable (Oscillator B)
  heap_caps_free(buffer_B);
  heap_caps_free(fbuffer_B);
