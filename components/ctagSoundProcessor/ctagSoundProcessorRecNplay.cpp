...
For each step recorded all current parameter-settings of the synth will be stored. Direct play or step playbach can make use of an Attack/Decay envelope.
...
#include "ctagSoundProcessorRecNplay.hpp"
...
float f_val_result = 0.f;
  ...
  if( t_StepperActive && t_RecPlay )    // Stepper active in Play-Mode?
  {
    if (t_StepAndEGtrigger == GATE_HIGH_NEW)
    {
      ...
      // ### printf("f_DownsampleFactorMem_: %f,SnHlatched_ == true \n", f_DownsampleFactorMem_);
      // ### fflush(stdout);
    }
  }
