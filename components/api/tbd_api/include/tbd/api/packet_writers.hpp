...
inline void write_packet_header_to_buffer(const Packet& packet, uint8_t* buffer) {
    ...
    buffer[Packet::OFFSET_HEADER_END + packet.payload_length + 1] = Packet::END_BYTE;
}
...
private:
    ...
    bool _write(const Packet& packet) {
        write_packet_header_to_buffer(packet, buffer_);
...
private:
    bool _send(const Packet& packet) {
        write_packet_header_to_buffer(packet, buffer_);
        stream_.put(buffer_, Packet::HEADER_SIZE);
