...
from tbd_core.reflection import ReflectableDescription, Reflectables
...
class PluginsOptions:
    ...
    plugins: list[ReflectableDescription] = field(default_factory=list)
    headers: set[Path] = field(default_factory=set)
    plugin_entries: list[PluginEntry] = field(default_factory=list)
    num_params: int = 0

class Plugins:
    def __init__(self, options: PluginsOptions):
        self._reflectables: Final = options.reflectables
        self._plugins: Final = options.plugins
        self._headers: Final = options.headers
        self._plugin_entries: Final = options.plugin_entries
